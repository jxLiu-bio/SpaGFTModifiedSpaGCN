...
class SpaGCN(object):
    ...
    def train(self,
        ...
        if emb2 is None:
            ...
            if issparse(adata.X):
                pca.fit(np.concatenate((adata.X.A, emb2), axis=1))
                embed=pca.transform(np.concatenate((adata.X.A, emb2),axis=1))
            else:
                pca.fit(np.concatenate((adata.X, emb2), axis=1))
                embed=pca.transform(np.concatenate((adata.X, emb2), axis=1))
        ...
        self.model.fit(embed,adj_exp,lr=self.lr,max_epochs=self.max_epochs,
                       weight_decay=self.weight_decay,opt=self.opt,init_spa=self.init_spa,
                       init=self.init,n_neighbors=self.n_neighbors,n_clusters=self.n_clusters,

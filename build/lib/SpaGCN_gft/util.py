...
def search_l(p, adj, start=0.01, end=1000, tol=0.01, max_run=100, ut=None):
    run=0
...
def search_res(adata, adj, l, target_num, emb2=None, start=0.4, step=0.1, tol=5e-3, lr=0.05, max_epochs=10, r_seed=100, t_seed=100, n_seed=100, max_run=10):
    random.seed(r_seed)
    ...
    clf=SpaGCN()
    clf.set_l(l)
    clf.train(adata,adj,emb2=emb2, init_spa=True,init="louvain",res=res, tol=tol, lr=lr, max_epochs=max_epochs)
    ...
    while old_num!=target_num:
        ...
        clf=SpaGCN()
        clf.set_l(l)
        clf.train(adata,adj, emb2=emb2, init_spa=True,init="louvain",res=res+step*old_sign, tol=tol, lr=lr, max_epochs=max_epochs)
        y_pred, _=clf.predict()
